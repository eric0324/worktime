...
def format(workbook, custom_format, need_center, need_border):
...
def generate_xlsx_result(year, month, name, number):
    ...
    # 4. 定義一個加粗的格式物件
    default_format = format(workbook, {}, True, True)
    bold = format(workbook, {'bold' : True, 'font_size' : 16}, True, False)
    bg_color = format(workbook, {'bg_color' : '#C0C0C0'}, True, True)
    text_wrap = format(workbook, {'text_wrap' : True}, False, True)
